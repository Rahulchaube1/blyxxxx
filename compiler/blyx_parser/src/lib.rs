...
impl BlyxParser {
    pub fn new(input: &str) -> Self {
        Self::with_tokens(BlyxLexer::new(input).tokenize())
    }
    pub fn with_file(input: &str, file: &str) -> Self {
        Self::with_tokens(BlyxLexer::with_file(input, file).tokenize())
    }
    ...
    fn peek(&self) -> &Token {
        self.tokens.get(self.pos).or_else(|| self.tokens.last()).expect("parser requires EOF token")
    }
    ...
    fn parse_fn(&mut self, is_pub: bool, is_async: bool) -> Option<FnDef> {
        ...
        Some(FnDef { name, generics, params, return_type, body, is_pub, is_async, span })
    }
        ...
        while !self.check(&TokenKind::Gt) && !self.is_at_end() {
            ...
            if let Some(name) = self.parse_ident() {
                ...
                if self.match_token(&TokenKind::Colon) {
                    if let Some(b) = self.parse_ident() {
                        bounds.push(b)
                    }
                }
                v.push(GenericParam { name, bounds, span })
            } else {
                self.sync_stmt();
                break;
            }
    ...
    fn parse_type(&mut self) -> BlyxType {
        let name = match self.peek().kind.clone() {
            ...
            TokenKind::SelfKw => {
                self.advance();
                return BlyxType::SelfType;
            }
            _ => {
                self.error("Expected a type");
                return BlyxType::Inferred;
            }
        };
        match name.as_str() {
            "u8" => BlyxType::U8,
    ...
    fn parse_struct(&mut self, is_pub: bool) -> Option<StructDef> {
        ...
        if self.match_token(&TokenKind::LBrace) {
            while !self.check(&TokenKind::RBrace) && !self.is_at_end() {
                if let Some(f) = self.parse_param() {
                    fields.push(f)
                }
                if !self.match_token(&TokenKind::Comma) {
                    break;
    ...
    fn parse_enum(&mut self, is_pub: bool) -> Option<EnumDef> {
        ...
        if self.match_token(&TokenKind::LBrace) {
            while !self.check(&TokenKind::RBrace) && !self.is_at_end() {
                if let Some(v) = self.parse_ident() {
                    variants.push(v)
                } else {
                    break;
                }
        ...
        if self.match_token(&TokenKind::LBrace) {
            while !self.check(&TokenKind::RBrace) && !self.is_at_end() {
                if self.check(&TokenKind::Fn) {
                    if let Some(m) = self.parse_fn(false, false) {
                        methods.push(m)
                    }
                } else {
                    self.error("Expected trait method");
                    self.sync_stmt()
                }
            }
    ...
    fn parse_block(&mut self) -> Option<Block> {
        ...
        while !self.check(&TokenKind::RBrace) && !self.is_at_end() {
            if let Some(s) = self.parse_stmt() { stmts.push(s) } else { self.sync_stmt() }
        }
    ...
    fn parse_stmt(&mut self) -> Option<Stmt> {
        if self.match_token(&TokenKind::Let) {
            ...
            let ty =
                if self.match_token(&TokenKind::Colon) { Some(self.parse_type()) } else { None };
            let value =
                if self.match_token(&TokenKind::Eq) { Some(self.parse_expr(0)) } else { None };
            if value.is_none() && ty.is_none() {
                self.error_at(span, "Expected a type or initializer in let declaration")
            }
            self.match_token(&TokenKind::Semi);
    ...
    fn parse_expr(&mut self, min_prec: u8) -> Expr {
        ...
        loop {
            let (op, prec) = match self.binary_operator() {
                Some(x) => x,
                None => break,
            };
            if prec < min_prec {
            ...
            left = Expr::Binary(op, Box::new(left), Box::new(right), self.previous_span())
        }
        left
    }
    fn binary_operator(&self) -> Option<(BinOp, u8)> {
        Some(match self.peek().kind {
            ...
            TokenKind::LtLt => (BinOp::Shl, 8),
            TokenKind::GtGt => (BinOp::Shr, 8),
    ...
    fn parse_prefix(&mut self) -> Expr {
        ...
        match t.kind {
            TokenKind::IntLit(v) => {
                self.advance();
                Expr::Literal(Lit::Int(v), t.span)
            }
            TokenKind::FloatLit(v) => {
                self.advance();
                Expr::Literal(Lit::Float(v), t.span)
            }
            TokenKind::StringLit(s) => {
                self.advance();
                Expr::Literal(Lit::String(s), t.span)
            }
            TokenKind::CharLit(c) => {
                self.advance();
            ...
            TokenKind::Ident(n) => {
                self.advance();
                self.parse_postfix(Expr::Ident(n, t.span))
            }
            TokenKind::LParen => self.parse_paren_or_tuple(),
            TokenKind::LBrack => self.parse_array(),
            TokenKind::Minus => {
                self.advance();
                Expr::Neg(Box::new(self.parse_expr(11)), t.span)
            }
            TokenKind::Bang => {
                self.advance();
                Expr::Unary(UnOp::Not, Box::new(self.parse_expr(11)), t.span)
            }
            ...
            TokenKind::Async => {
                ...
                if self.check(&TokenKind::Pipe) {
                    ...
                    self.error("Expected closure after 'async'");
                    Expr::default()
                }
            }
        ...
        loop {
            if self.match_token(&TokenKind::LParen) {
                ...
                let args = if self.match_token(&TokenKind::LParen) {
                    ...
                    let count = self.pos.saturating_sub(start);
    ...
    fn parse_paren_or_tuple(&mut self) -> Expr {
        ...
        if self.match_token(&TokenKind::RParen) {
            return Expr::TupleLit(Vec::new(), span);
        }
    ...
    fn parse_if(&mut self) -> Expr {
        ...
        let else_branch = if self.match_token(&TokenKind::Else) {
            if self.check(&TokenKind::If) {
                ...
                Some(Box::new(Expr::Block(self.parse_block().unwrap_or_default())))
            }
        } else {
            None
        };
    ...
    fn parse_match(&mut self) -> Expr {
        ...
        while !self.check(&TokenKind::RBrace) && !self.is_at_end() {
            ...
            let guard =
                if self.match_token(&TokenKind::If) { Some(self.parse_expr(0)) } else { None };
    ...
    fn parse_pattern(&mut self) -> Pattern {
        ...
        match t.kind {
            TokenKind::IntLit(v) => {
                self.advance();
                Pattern::Literal(Lit::Int(v), t.span)
            }
            TokenKind::FloatLit(v) => {
                self.advance();
                Pattern::Literal(Lit::Float(v), t.span)
            }
            TokenKind::StringLit(s) => {
                self.advance();
                Pattern::Literal(Lit::String(s), t.span)
            }
            TokenKind::CharLit(c) => {
                self.advance();
            ...
            TokenKind::Ident(n) => {
                self.advance();
                if n == "_" { Pattern::Wildcard(t.span) } else { Pattern::Ident(n, t.span) }
            }
            TokenKind::LParen => {
                self.advance();
                let mut p = Vec::new();
                ...
                Pattern::Tuple(p, t.span)
            }
            _ => {
                self.error("Expected match pattern");
                self.advance();
                Pattern::Wildcard(t.span)
            }
        }
    }
    fn parse_ai_call(&mut self, generate: bool) -> Expr {
        ...
        let first = self.parse_expr(0);
        let second =
            if self.match_token(&TokenKind::Comma) { Some(self.parse_expr(0)) } else { None };
        self.expect(&TokenKind::RParen, "Expected ')' after AI operation");
        if generate {
            let prompt = second.unwrap_or_else(|| Expr::default());
            Expr::Generate { model: Box::new(first), prompt: Box::new(prompt), span }
        } else {
            Expr::Reason { context: Box::new(first), span }
        }
    }
    fn parse_orchestrate(&mut self) -> Expr {
        ...
        while !self.check(&TokenKind::RParen) && !self.is_at_end() {
            agents.push(self.parse_expr(0));
            if !self.match_token(&TokenKind::Comma) {
                break;
            }
        }
        self.expect(&TokenKind::RParen, "Expected ')' after orchestrate");
        Expr::Orchestrate { agents, task: Box::new(Expr::default()), span }
    }
    fn parse_closure(&mut self, is_async: bool) -> Expr {
        ...
        while !self.check(&TokenKind::Pipe) && !self.is_at_end() {
            ...
            if let Some(n) = self.parse_ident() {
                params.push(ClosureParam { name: n, ty: None, span: pspan })
            }
            if !self.match_token(&TokenKind::Comma) {
                break;
            }
...
mod tests {
    ...
    #[test]
    fn test_parse_fn() {
        let mut p = BlyxParser::new("fn main() {}");
        let a = p.parse_file("test.blyx");
        assert_eq!(p.errors().len(), 0);
        assert_eq!(a.items.len(), 1)
    }
    #[test]
    fn test_parse_async_fn() {
        let mut p = BlyxParser::new("async fn foo() {}");
        p.parse_file("test.blyx");
        assert_eq!(p.errors().len(), 0)
    }
    #[test]
    fn test_parse_struct() {
        let mut p = BlyxParser::new("struct Foo { x: i32 }");
        p.parse_file("test.blyx");
        assert_eq!(p.errors().len(), 0)
    }
    #[test]
    fn test_parse_actor() {
        let mut p = BlyxParser::new("actor Worker {}");
        p.parse_file("test.blyx");
        assert_eq!(p.errors().len(), 0)
    }
    #[test]
    fn test_parse_generate() {
        let mut p = BlyxParser::new("fn foo() { generate(m, p); }");
        p.parse_file("test.blyx");
        assert_eq!(p.errors().len(), 0)
    }
    #[test]
    fn test_parse_reason() {
        let mut p = BlyxParser::new("fn foo() { reason(ctx); }");
        p.parse_file("test.blyx");
        assert_eq!(p.errors().len(), 0)
    }
    #[test]
    fn test_parse_task() {
        let mut p = BlyxParser::new("task do_work() {}");
        p.parse_file("test.blyx");
        assert_eq!(p.errors().len(), 0)
    }
    #[test]
    fn test_parse_match() {
        let mut p = BlyxParser::new("fn foo() { match x { 1 => 2 } }");
        p.parse_file("test.blyx");
        assert_eq!(p.errors().len(), 0)
    }
    #[test]
    fn test_parse_closures() {
        let mut p = BlyxParser::new("fn foo() { let f = |x| x + 1; }");
        p.parse_file("test.blyx");
        assert_eq!(p.errors().len(), 0)
    }
    #[test]
    fn test_parse_let() {
        let mut p = BlyxParser::new("fn foo() { let mut x: i32 = 5; }");
        p.parse_file("test.blyx");
        assert_eq!(p.errors().len(), 0)
    }
    #[test]
    fn test_parse_binary() {
        let mut p = BlyxParser::new("fn foo() { 1 + 2 * 3; }");
        p.parse_file("test.blyx");
        assert_eq!(p.errors().len(), 0)
    }
    #[test]
    fn test_parse_macro() {
        let mut p = BlyxParser::new("fn foo() { println!(\"hello\"); }");
        p.parse_file("test.blyx");
        assert_eq!(p.errors().len(), 0)
    }
    #[test]
    fn test_parse_if() {
        let mut p = BlyxParser::new("fn foo() { if true {} else {} }");
        p.parse_file("test.blyx");
        assert_eq!(p.errors().len(), 0)
    }
    #[test]
    fn test_parse_while() {
        let mut p = BlyxParser::new("fn foo() { while true {} }");
        p.parse_file("test.blyx");
        assert_eq!(p.errors().len(), 0)
    }
    #[test]
    fn test_error_recovery() {
        let mut p = BlyxParser::new("fn foo() { let = ; } fn bar() {}");
        let a = p.parse_file("test.blyx");
